class HistogramInterpreter(object) :
  '''ROOT to numpy array converters'''
  def __init__( self ) :
    pass

  def to_array( self, histogram, x_range=None, y_range=None ) :
    ...
    for bin in range(histogram.GetNbinsX()+1) :
      if x_range is not None :
        if x_range[0] is not None and histogram.GetBinCenter(bin) < x_range[0] : continue
        if x_range[1] is not None and histogram.GetBinCenter(bin) > x_range[1] : continue
      if y_range is not None :
        if y_range[0] is not None and histogram.GetBinCenter(bin) < y_range[0] : continue
        if y_range[1] is not None and histogram.GetBinCenter(bin) > y_range[1] : continue
